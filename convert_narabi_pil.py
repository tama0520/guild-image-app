...
# _patch_and_run_narabi() が regex で書き換える。記事用（新宿歌舞伎町）だけ
# リポジトリ同梱の fonts/NotoSansJP-Black.otf を指すので、ローカル・Cloud とも
# ⑦プレビューと同じ書体になる。他店舗・他ページは空のまま＝従来書体。
FONT_OVERRIDE = ""
...
def _load_font(size):
    # 差し替え指定があり実在するときだけ使う（無ければ FONT_PATH へ落ちる）。
    try:
