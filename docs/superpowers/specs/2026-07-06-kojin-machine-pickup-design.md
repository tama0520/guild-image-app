# 個別機種の優秀台ピックアップ セクション 設計

作成日: 2026-07-06

## 背景・目的

②個別画像の「その他の優秀台ピックアップ」台番テキスト欄に、バラバラの機種の台番をまとめて貼って合体画像を作っている。だが、そこに含まれるジャグラー機種（例: マイジャグV）がパイプラインで自動的に「高配分」と判定され、単独の `{機種名}_高配分.jpg` が自動生成されてしまう。

⑦プレビューでその自動高配分画像のチェックを外すと、kojin/uncheck ルーティング（`streamlit_app.py` の 7254–7276 付近および 7082 以降の分岐）によってジャグラー機種がジャグラーシリーズ優秀台.jpg に合流してしまう。ユーザーはこれを望まない。

**根本対策**: 手動で台をピックアップしたい機種は、そもそも自動高配分画像を作らせない。作られなければプレビューに出ず、チェック外しも合流も発生しない。

## スコープ

新セクション「個別機種の優秀台ピックアップ」を②個別画像に追加する。全店舗共通。**記事用ページ（`art_` プレフィックス、高田馬場専用）も対象**。

## UI 仕様

- **枠数**: 3個。各枠に次の2欄:
  - **タイトル**（自由入力・専用欄）
  - **台番テキスト**（台番を含むテキストを貼り付け。`expand_machine_numbers` で台番を抽出）
- 機種名の入力欄は**設けない**（抑制対象機種は台番から逆引きする）。

### 通常ページ（`show_auto_page`）

- **配置**: ②個別画像セクション内、「その他の優秀台ピックアップ」の**すぐ上**。`kojin_enabled`（個別画像も生成する）が ON のときのみ表示。
- 入力は `_save_auto_inputs` で永続化。キー命名:
  - `kojin_pick_title_{i}_{store}` / `kojin_pick_bans_{i}_{store}`（i = 0..2）

### 記事用ページ（`show_auto_article_page`・高田馬場専用）

- 記事用ページには「その他の優秀台ピックアップ」欄が無いため、②個別画像の**並び台番範囲欄の下（③並び画像の直前）**に配置。`art_kojin_enabled` が ON のときのみ表示。
- 入力は `_save_article_inputs` で永続化。`_save_article_inputs` の `keys` に追加。キー命名:
  - `art_kojin_pick_title_{i}_{store}` / `art_kojin_pick_bans_{i}_{store}`（i = 0..2）
- 画像描画は記事用に合わせ、青タイトルバー・ピンクバーとも無しの `_build_machine_img_no_bar(df)` を使用。

## 生成画像仕様

各枠について、台番テキストが非空なら:

1. 貼った台番に一致する行を Excel（`_pv_df`）から抽出。
2. 台番順にソート。
3. 描画（**ピンクバーなし**、タイトル＝専用欄の文字列）:
   - 通常ページ: `_build_machine_img(df, title, None)`（`summary_stat=None`）
   - 記事用ページ: `_build_machine_img_no_bar(df)`（青タイトルバー・ピンクバー無し）
4. ファイル名 = `{_make_safe_fn(title)}.jpg`。
5. ⑦プレビューの一覧に追加（並び画像と同じ扱いの位置。個別画像＝②の並びに合わせて配置）。

抽出条件は**優秀台フィルタなし**（貼った台番をそのまま載せる）。

## 自動高配分の抑制

各枠の台番から機種を逆引き（`_pv_df` の 台番→機種名）して**抑制対象機種セット**を作り、パイプラインへ渡す:

- `run_step2_juggler` / `run_step3_other` の自動高配分生成を抑制するため、既存の `recommended_machines`（＝個別画像指定として高配分・統合から除外される仕組み）に、この逆引き機種を **合流させる**。
- **`sonota_exclude` には加えない**。`recommended_machines` への追加だけで自動その他ピックアップからも除外される（Step3 の 3080/3089 行、およびジャグラー系は Step3 冒頭で除外）。`sonota_exclude` にジャグラー機種を入れると `run_step2_juggler` の 3007 行（`sonota_exclude & _juggler_names` でジャグラー統合を作らず全台 overflow）が誤発火し、無関係なジャグラー台がまるごとその他へ流れてしまうため。

これにより:
- 対象機種の `_高配分.jpg` は生成されない → ⑦プレビューに出ない → チェック外しが起きない → ジャグラーシリーズ優秀台.jpg / その他の優秀台ピックアップ.jpg への合流も起きない（今回の問題が根本解消）。

## 重複についての前提

同じ機種の台番を、この新欄と既存「その他の優秀台ピックアップ」台番欄の**両方に書くことは基本ない**（ユーザー確認済み）。したがって両者間の重複除去は必須要件としない。

## 反映箇所

通常ページ:
- ⑦プレビュー生成（`show_auto_page` のプレビュー構築ブロック、6180–6889 付近）
- ⑧実行（`run_auto_pipeline` 呼び出しと出力ZIP構築）
- 入力の永続化（`_save_auto_inputs` / `auto_page_inputs.json`）

記事用ページ（高田馬場専用）:
- プレビュー生成（`show_auto_article_page` のプレビュー構築ブロック、10225 付近〜）
- 実行・出力
- 入力の永続化（`_save_article_inputs` / `article_page_inputs.json`）

## 非対象（YAGNI）

- 機種名の明示入力欄
- 優秀台条件でのフィルタ
- 新欄と既存その他欄の台番重複の自動解決
